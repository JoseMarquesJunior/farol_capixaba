...
dados_demograficos = renomear_colunas_demografia(dados_demograficos)
gdf = merge_setores_com_dados_demograficos(gdf, dados_demograficos)
gdf = encontrar_centroides(gdf)
gdf.drop(columns="geometry").to_excel("educacao/data/ES_setores_com_dados_demograficos.xlsx", index=False)
gdf = configurando_gdf_para_plotly(gdf)
fig = plotar_setores(gdf)
#fig.show()
...
df_escola = merge_escolas_com_matriculas(df_escola, df_matricula)
df_escola = renomear_colunas_matriculas(df_escola)
df_escola.to_excel("analises/escolas_ES_ativas.xlsx", index=False)
fig = plotar_escolas(fig, df_escola)
fig.show()

fig.write_html("analises/setores_censitarios_mais_dados_demograficos.html")